"""
Script for testing the multiframe segmentation model on a single video.
Can be used for tool-tip/pose segmentation.
Author: Bhargav Ghanekar
Refactored for single-file processing.
"""
# --- ADDED: Import time module for measurements ---
import time
...
def main():
    parser = argparse.ArgumentParser(description='Test Multiframe Segmentation Model on a Single Video')
    # CHANGED: Replaced directory inputs with specific file inputs
    parser.add_argument('--rgb_video', type=str, required=True,
                        help='Path to the input RGB video file.')
    parser.add_argument('--depth_video', type=str, default=None,
                        help='(Optional) Path to the corresponding depth video file.')
    parser.add_argument('--output_dir', type=str, required=True,
                        help='Path to the directory where the output will be saved.')
    
    # Model and processing arguments (mostly unchanged)
    ...
    parser.add_argument('--add_optflow_inputs', action='store_true',
                        help='Add optical flow inputs to the model. Default: False')
    args = parser.parse_args()
    
    # ADDED: Logic to automatically set 'add_depth_inputs' flag
    args.add_depth_inputs = (args.depth_video is not None)
...
def compute_centroids_and_store(type, mask_array, output, centroid_locations, count, args, disp_image, prev_left_pose_detected_tips, cX_prev_left, cY_prev_left):
    if type=='left':
        ...
        left_tip_heatmap = output[0,4,:,:].cpu().to(torch.float32).numpy()
    elif type=='right':
        idxt1 = 4
        idxt2 = 5
        idxt3 = 6
...
def track_on_video(video_path, depth_video_path, model, args, logger, optflow_model=None):
    ...
    if args.add_depth_inputs:
        ...
        fname_depth = Path(depth_video_path).stem
    ...
    output_video_path = os.path.join(args.output_dir, f'{fname}_tracked.mp4')
    video = cv2.VideoWriter(output_video_path, fourcc, output_fps, (args.input_width, args.input_height))
    ...
    # --- ADDED: Initialize variables for timing ---
    inference_times = []
    total_processing_start_time = time.perf_counter()

    with torch.no_grad():
        tq = tqdm.tqdm(total=N) # CHANGED: Progress bar now tracks total frames
        tq.set_description(f"Processing {fname}")
        while True:
            ...
            # Skip processing until the buffer is full
            ...
            if len(image_queue) == args.num_input_frames:
                ...
                if torch.cuda.is_available():
                    ...
                    if args.add_depth_inputs:
                        input_depth = [d.cuda() for d in input_depth]
                ...
                with autocast():
                    if args.add_optflow_inputs:
                        ...
                        for i in range(1, len(input_tensors)):
                            frame_prev = F.interpolate(input_tensors[0], scale_factor=0.5, mode='nearest')
                            frame_curr = F.interpolate(input_tensors[i], scale_factor=0.5, mode='nearest')
                            flow = optflow_model(frame_prev, frame_curr)[-1]
                            flow = F.interpolate(flow/0.5, size=(input_tensors[0].size(2), input_tensors[0].size(3)), mode='bilinear', align_corners=True)
                            optflow.append(flow)
                        if args.add_depth_inputs:
                            output = model(input_tensors, optflow=optflow, depth=input_depth)
                        else:
                            output = model(input_tensors, optflow=optflow)
                    else:
                        if args.add_depth_inputs:
                            output = model(input_tensors, depth=input_depth)
                        else:
    ...
    # --- ADDED: Stop the total timer and calculate metrics ---
    total_processing_end_time = time.perf_counter()
    total_time_elapsed = total_processing_end_time - total_processing_start_time
    num_processed_frames = len(inference_times)

    cv2.destroyAllWindows()
    # video.release()
    
    output_csv_path = os.path.join(args.output_dir, f'{fname}_tracked.csv')
    np.savetxt(output_csv_path, centroid_locations, delimiter=',')
    logger.info(f"Saved tracked video to {output_video_path}")
    logger.info(f"Saved tracked results to {output_csv_path}")
    ...
    # --- ADDED: Log the final performance metrics ---
    if num_processed_frames > 0:
        avg_fps = num_processed_frames / total_time_elapsed
        avg_inference_time_ms = np.mean(inference_times) * 1000
        
        logger.info("================ Performance Metrics ================")
        logger.info(f"Total frames processed: {num_processed_frames}")
        logger.info(f"Total processing time: {total_time_elapsed:.2f} seconds")
        logger.info(f"Overall throughput: {avg_fps:.2f} FPS")
        logger.info(f"Average inference time per frame: {avg_inference_time_ms:.2f} ms")
        logger.info("=====================================================")
    return
...
def main_worker(args):
    ...
    # set up logging
    log_file_path = args.output_dir / "log.log"
    ...
    logger.addHandler(stream_handler) # ADDED: Log to console as well
    ...
    # logger.info("Compiling the model with torch.compile... (This may take a moment)")
    # model = torch.compile(model)
    # logger.info("Model compiled successfully.")

    logger.info(f"Processing RGB video: {args.rgb_video}")
    if args.add_depth_inputs:
        logger.info(f"Processing Depth video: {args.depth_video}")

    track_on_video(args.rgb_video, args.depth_video, model, args, logger, optflow_model)
