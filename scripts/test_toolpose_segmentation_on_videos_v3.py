...
def main():
    ...
    parser.add_argument('--video_fname_id', type=str, default=None,
                        help='text identifier in the video filenames to process')
        ...
        for root, dirs, files in os.walk(directory):
            for file in files:
                if file.endswith(('.mp4', '.avi', '.mov')):
                    if str_id is None or str_id in file:
                        lst.append(os.path.join(root, file))
        return sorted(lst)
    video_files = list_video_files(args.videos_dir, args.video_fname_id)
